...
def get_token(p):
    tokens = global_structure["tokens"]
    if p == len(tokens):
        return R(False, p, [])
    return R(True, p + 1, [tokens[p]])
def check_token(good):
    pp = lambda x, p: x if good(x.r[0]) else R(False, p, [])
    return post(pp, get_token)
...
def parse_lex(tokens):
    global_structure["tokens"] = tokens
...
def parse_yacc(tokens):
    ...
    def pdef(r):
        global_structure["defined_terms"].add(flatten(r)[0])
    ...
    undefined_terms = sorted(list(used_terms - global_structure["defined_terms"]))
